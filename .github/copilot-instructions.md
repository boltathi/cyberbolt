...
- **⚠️ CRITICAL RULE — LLM tools are NEVER public.** Any feature that calls Ollama (or any LLM) must be admin-only: `@admin_required()` on the API endpoint AND a frontend auth check (`useAuthStore().isAuthenticated`) on the UI page. LLM endpoints must never be exposed without authentication — they consume server resources and could be abused.
